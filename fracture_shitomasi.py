...
# finding shi tomasi corners by finding 20 best corners
shitomasi_img = cv2.goodFeaturesToTrack(canny_img,20,0.05,10)
shitomasi_img = np.int0(shitomasi_img)

# marking corners on canny edge detected image
can2img = cv2.imread('canny1.jpg')

for i in shitomasi_img:
    x,y = i.ravel()
    cv2.circle(can2img,(x,y),3,255,-1)

# displaying image
cv2.imwrite('shitomasi1.jpg',can2img)
finalshitomasi = cv2.imread('shitomasi1.jpg')
 
f, plots = plt.subplots() 
plots.imshow(finalshitomasi)
plt.show()
